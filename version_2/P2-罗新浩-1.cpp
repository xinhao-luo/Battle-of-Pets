...
int rnum = 1, wnum = 0;
int pski = 1, eski = 1, pd = 0, ed = 0, pd1 = 0, ed1 = 0;
...
void enemyAtk(ostream& os, Pet p)
{
    ...
    if(nowe.getType() == D){
        if(p.getHP() < int(p.maxHP() * 1.0 / 3 + 0.5)){
            if(p.petAsleep() || p.petPaint()){
                os<<"Enemy's D uses Slap! Damage: "<<Damage(nowe, nowp, 1)<<endl;
                ed = 1;
                ed1 = Damage(nowe, nowp, 1);
            }else{
                os<<"Enemy's D uses Sing! ";
                if(nowp.petAsleep())
                    os<<"Waste"<<endl;
                else{
                    os<<"Your "<<nowp.strType()<<" falls asleep"<<endl;
                    nowp.fallAsleep();
            ...
            if(!p.petAsleep()){
                ...
                if(!p.petPaint()){
                    ...
                    if(pski == 2)
                        ...
                        os<<"Your "<<nowp.strType()<<" is painted"<<endl;
                        nowp.bePainted();
                        if(nowp.getType() == np1.getType())
                            np1.bePainted();
                        else if(nowp.getType() == np2.getType())
    ...
    }else if(nowe.getType() == L){
        if(p.getHP() < int(p.maxHP() * 1.0 / 5 +0.5)){
            if(p.getType() == L || p.getType() == G){
                os<<"Enemy's L uses Claw! Damage: "<<Damage(nowe, nowp, 1)<<endl;
                ed = 1;
                ed1 = Damage(nowe, nowp, 1);
            }else{
                if(w != 2){
                    ...
                    os<<"Enemy's L uses Flame! Damage: "<<Damage(nowe, nowp, 2)<<endl;
                    ed = 2;
        ...
        if(nowe.getHP() >= 55 || (nowe.getHP() > p.getHP() && !is_alive(e2, e3))){
            if(p.getType() == G){
...
int enemyPri()
{
    if(nowe.getType() == D)
        ...
        if(nowp.getHP() < nowp.maxHP() * (1 / 5)){
            if(nowp.getType() == L || nowp.getType() == G)
                return 0;
            else{
                if(w != 2)
                    return 1;
...
void nextPet(istream& is, ostream& os)
{
    ...
    int n;
    is >> n;
    if(nowe.getType() == W && nowe.getHP() < 55 && e2.petAlive()){
...
{
    if(nowe.getType() == D && e1.petAlive())
        return W;
    else if(nowe.getType() == L && e1.petAlive())
        return W;
    else
        return L;
}
